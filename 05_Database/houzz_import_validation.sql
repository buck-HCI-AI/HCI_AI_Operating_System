...
-- Finds items with same houzz_item_id (primary key constraint should catch, but verify)
SELECT 'duplicate_schedule_items' as check_name,
       houzz_item_id, COUNT(*) as duplicate_count
FROM houzz_schedule_items
GROUP BY houzz_item_id
HAVING COUNT(*) > 1;
...
FROM houzz_schedule_items WHERE project_id IS NULL;
...
FROM houzz_schedule_items child
WHERE child.parent_item_id IS NOT NULL
  AND NOT EXISTS (
      SELECT 1 FROM houzz_schedule_items parent
      WHERE parent.houzz_item_id = child.parent_item_id
  );
